...
CREATE TABLE catalogo_periodo_academico (
    ...
    ESTADO_PLANIFICACION    VARCHAR(15)     NOT NULL DEFAULT 'BORRADOR',
...
CREATE TABLE docente (
    ...
    TIPO_DOCUMENTO          VARCHAR(12)     NOT NULL DEFAULT 'CEDULA',
    ...
    CONSTRAINT FK_DOC_DEDICACION foreign KEY (ID_DEDICACION) 
        ...
        ON update cascade ON delete RESTRICT,
    CONSTRAINT CHK_DOC_DOCUMENTO_FORMATO CHECK (
    ...
    CONSTRAINT CHK_DOC_TIPO_SANGRE CHECK (
        TIPO_SANGRE IS NULL OR BTRIM(TIPO_SANGRE) IN ('A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-')
    ),
...
CREATE TABLE catalogo_actividad_complementaria (
    ID_ACTIVIDAD       SERIAL          NOT NULL,
    CODIGO_ACTIVIDAD   VARCHAR(20)     NOT NULL,
    NOMBRE_ACTIVIDAD   VARCHAR(150)    NOT NULL,
    TIPO_ACTIVIDAD     VARCHAR(20)     NOT NULL DEFAULT 'COMPLEMENTARIA',
    ACTIVIDAD_ACTIVA   BOOLEAN         NOT NULL DEFAULT TRUE,
    CONSTRAINT PK_CATALOGO_ACTIVIDAD PRIMARY KEY (ID_ACTIVIDAD),
    CONSTRAINT UK_CATALOGO_ACTIVIDAD_CODIGO UNIQUE (CODIGO_ACTIVIDAD),
    CONSTRAINT CHK_CATALOGO_ACTIVIDAD_TIPO CHECK (
        TIPO_ACTIVIDAD IN ('COMPLEMENTARIA', 'INVESTIGACION', 'GESTION', 'VINCULACION')
    )
);

CREATE TABLE planificacion_actividad_docente (
    ID_ACTIVIDAD_DOCENTE BIGSERIAL       NOT NULL,
    ID_DOCENTE           BIGINT          NOT NULL,
    ID_CARRERA           INTEGER         NOT NULL,
    ID_PERIODO           INTEGER         NOT NULL,
    ...
    CONSTRAINT PK_ACTIVIDAD_DOCENTE PRIMARY KEY (ID_ACTIVIDAD_DOCENTE),
    CONSTRAINT UK_ACTIVIDAD_DOCENTE_PERIODO UNIQUE (ID_DOCENTE, ID_PERIODO, ID_ACTIVIDAD),
    CONSTRAINT CHK_ACTIVIDAD_HORAS CHECK (HORAS_ASIGNADAS > 0),
    CONSTRAINT FK_ACT_DOCENTE FOREIGN KEY (ID_DOCENTE) REFERENCES docente (ID_DOCENTE) ON DELETE RESTRICT,
    CONSTRAINT FK_ACT_CARRERA FOREIGN KEY (ID_CARRERA) REFERENCES catalogo_carrera (ID_CARRERA) ON DELETE RESTRICT,
    CONSTRAINT FK_ACT_PERIODO FOREIGN KEY (ID_PERIODO) REFERENCES catalogo_periodo_academico (ID_PERIODO) ON DELETE RESTRICT,
    CONSTRAINT FK_ACT_CATALOGO FOREIGN KEY (ID_ACTIVIDAD) REFERENCES catalogo_actividad_complementaria (ID_ACTIVIDAD) ON DELETE RESTRICT
);
...
CREATE TABLE planificacion_asignacion_docente (
    ...
    SEMANAS_PLANIFICADAS        SMALLINT        NOT NULL DEFAULT 16,
    COMISION_SERVICIO           VARCHAR(100)    NULL,
    CONSTRAINT PK_PLAN_ASIGNACION_DOCENTE PRIMARY KEY (ID_ASIGNACION),
    CONSTRAINT UK_PAD_ASIG_CARRERA_PERIODO_PARALELO
        UNIQUE (ID_ASIGNATURA, ID_CARRERA, ID_PERIODO, PARALELO_ASIGNADO),
...
CREATE TABLE planificacion_aula_horario (
    ...
    ID_ASIGNACION   BIGINT          NULL,
    DIA_SEMANA      SMALLINT        NOT NULL DEFAULT 1,
    HORA_INICIO     TIME            NULL,
    HORA_FIN        TIME            NULL,
    CONSTRAINT PK_PLAN_AULA_HORARIO PRIMARY KEY (ID_HORARIO),
    CONSTRAINT UK_PAH_PERIODO_AULA_DIA_INICIO UNIQUE (ID_PERIODO, NOMBRE_AULA, DIA_SEMANA, HORA_INICIO),
    CONSTRAINT FK_PAH_PERIODO FOREIGN KEY (ID_PERIODO) 
        REFERENCES catalogo_periodo_academico (ID_PERIODO)
        ON update cascade ON delete RESTRICT,
    CONSTRAINT FK_PAH_ASIGNACION FOREIGN KEY (ID_ASIGNACION)
        REFERENCES planificacion_asignacion_docente (ID_ASIGNACION)
        ON update cascade ON delete RESTRICT,
